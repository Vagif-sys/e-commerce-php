...
if(isset($_POST['search'])){
    ...
    // if user just entered the page then default page is one 
    }else{
   ...
   $products = $pdo->prepare("SELECT COUNT(*) As total_records FROM products WHERE product_category=:p_category AND product_price<=:p_price");
   $products->execute(array('p_category'=>$category,'p_price'=>$price));
   $total_records = $products->fetchColumn();
    ...
    $products = $pdo->prepare("SELECT * FROM products WHERE product_category=:p_category AND product_price<=:p_price LIMIT $offset,$total_records_per_page");
    $products->execute(array('p_category'=>$category,'p_price'=>$price));
    //$products = $stmt2->get_result(); 
...
//  return all products   
   ...
   $products = $pdo->prepare("SELECT COUNT(*) As total_records FROM products");
   $products->execute();
   $total_records = $products->fetchColumn();
    ...
    $products = $pdo->prepare("SELECT * FROM products LIMIT $offset,$total_records_per_page");
    $products->execute();
    //$products = $stmt2->get_result(); 
}
